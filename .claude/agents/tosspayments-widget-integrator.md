...
## 주의사항: 해시 라우팅 successUrl (Hash Routing successUrl Pitfall)

**증상 (Symptom):**
- 결제 완료 후 success 화면에서 `paymentKey` / `orderId` / `amount`가 모두 `undefined`로 들어와 confirm API를 호출하지 못함
- 클라이언트가 "결제 정보가 올바르지 않습니다" 등의 에러 분기로 빠짐
- 사용자는 실제로 카드 결제까지 마친 상태에서 발생하므로 영향이 큼

**원인 (Root Cause):**
토스 결제위젯 SDK는 `successUrl`을 그대로 redirect하지 않고, URL을 파싱한 뒤 query를 자체적으로 재조립한다. 이때 `successUrl`에 hash(`#`)가 포함되어 있으면 브라우저 표준 URL 파싱 규칙에 따라:
- `paymentKey` / `orderId` / `amount` 등 토스가 부여하는 파라미터는 **hash 앞의 `location.search`**에 붙는다
- SDK가 별도로 추가하는 `paymentType=NORMAL`만 **hash 안의 query string**에 보존된다

따라서 hash 안의 query string만 파싱하는 헬퍼는 핵심 파라미터를 못 본다. 이는 React Router `HashRouter`, Next.js의 hash 사용, vanilla hash routing 등 **모든 hash 기반 SPA 라우팅에 동일하게 적용**된다.

**실제 redirect URL 예시:**
```
http://localhost:3000/?orderId=lumen-...&paymentKey=tgen_...&amount=49000#/payments/success?paymentType=NORMAL
```

이때:
- `location.search` → `?orderId=...&paymentKey=...&amount=49000`
- hash 안의 query (`#/payments/success?paymentType=NORMAL` 뒤쪽) → `?paymentType=NORMAL`

**해결 (Fix):**
success 페이지의 query 파싱 헬퍼가 `location.search`와 hash 안 query string **둘 다** `URLSearchParams`로 파싱해서 병합해야 한다.

**권장 헬퍼 코드:**
```javascript
// Hash routing safe query parser — merges location.search + hash query
function parseHashQuery() {
  const params = new URLSearchParams();

  // 1. Parse location.search (where Toss puts paymentKey/orderId/amount)
  const search = new URLSearchParams(window.location.search);
  search.forEach((value, key) => params.set(key, value));

  // 2. Parse query string inside the hash (where SDK puts paymentType etc.)
  const hash = window.location.hash; // e.g. "#/payments/success?paymentType=NORMAL"
...
// Usage in success route handler
...
**추가 권고 (Additional Recommendations):**

1. **confirm 성공 후 URL 정리**: confirm이 성공하면 `window.history.replaceState`로 `location.search`를 비워라. 이렇게 하면:
   - 사용자가 새로고침해도 `paymentKey` 등이 다시 confirm으로 보내지지 않음 (재confirm 방지)
   - URL에 민감 파라미터가 노출되는 시간을 최소화
...
2. **가능하면 hash 라우팅을 피하라**: 결제 successUrl로 들어오는 경로만이라도 history API 기반 라우팅(BrowserRouter, Next.js 일반 라우팅)으로 분리하는 것이 가장 깔끔하다. hash 라우팅이 불가피한 SPA(정적 호스팅, 일부 임베딩 환경)에서만 위 헬퍼를 사용하라.

3. **failUrl도 동일**: `failUrl`도 같은 방식으로 hash 앞 search에 `code` / `message` / `orderId`가 붙으므로 fail 화면에서도 같은 헬퍼를 사용해야 한다.
