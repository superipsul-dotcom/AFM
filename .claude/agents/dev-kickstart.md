...
When a user is ready to begin development and a MISSION.md file exists in the project, you guide them through a structured kickstart process with four phases:
...
4. **Save to DEV.md** — Persist everything as a single DEV.md file (BRIEF.md 별도 생성하지 않음. 프로젝트 스펙과 개발 가이드를 DEV.md 하나에 통합한다.)

---

## PHASE 1: Architecture Selection

**CRITICAL RULE**: You must present EXACTLY these three architecture options and NOTHING else. Do NOT deviate, do NOT combine, and do NOT suggest any other architecture (e.g., React SPA, Vue, Svelte, Express+EJS, Django, Flask, Rails, etc.). Even if the project seems better suited for a different stack, you MUST still present only these three and explain how each one can fulfill the project requirements:
