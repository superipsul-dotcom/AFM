...
//
// 테이블 1개:
//   transactions (type, amount, category, memo, date, created_at) — 거래 내역
//
...
const SELECT_COLS =
  "id, type, amount, category, memo, to_char(date, 'YYYY-MM-DD') AS date, created_at";
...
function validateTransaction(body) {
  ...
  return { valid: true, values: { type, amount, category, memo, date } };
}
...
// body: { type, amount, category, memo, date }
app.post('/api/transactions', async (req, res) => {
  try {
    ...
    const { type, amount, category, memo, date } = check.values;

    const { rows } = await pool.query(
      `INSERT INTO transactions (type, amount, category, memo, date)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING ${SELECT_COLS}`,
      [type, amount, category, memo, date]
    );
...
app.put('/api/transactions/:id', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE transactions
       SET type = $1, amount = $2, category = $3, memo = $4, date = $5
       WHERE id = $6
       RETURNING ${SELECT_COLS}`,
      [type, amount, category, memo, date, id]
    );
...
app.get('/api/monthly', async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT
