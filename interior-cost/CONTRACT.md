...
# v29 — ☑️ 공정 선택 위저드 + 스케치업 물량 자동 선택 (2026-07-19)

## 29-1 UI (프론트 전용 — 신규 서버 엔드포인트 없음)
- **견적 항목 툴바에 [☑️ 공정 선택하기]** (임시저장/물량/카탈로그/행추가 옆, primary). → `ProcessWizardModal`.
- **위저드 흐름**: 1단계 공종 체크(카탈로그 보유 공종 그리드, 항목 수·선택 수 표기) → [다음]마다 선택 공종별 스텝(TRADE_MASTER 순, 스텝 내 검색·전체선택/해제·체크 시 수량 인라인 편집) → 마지막 요약 단계(공종별 정렬 미리보기) → [✅ 완료 (n개 추가)] 일괄 행 추가. 진행칩 클릭으로 스텝 점프. 푸터에 선택 수·합계(부가세 전) 실시간.
- **행 추가 규칙**: 카탈로그 항목 → `{trade,name,unit,spec(product_name||grp),qty,재/노/부,catalog_id}` (pickCatalog 와 동형). 최초 빈 행 1개뿐이면 대체. 1단계에서 공종 해제 시 그 공종의 체크 자재 정리(goNext 프루닝).
- **📐 스케치업/실측 물량 자동 선택**: 위저드 오픈 시 `listTakeoff + resolveMaterialCodes` — 공종 = takeoff.trade ∪ resolve.trade, 자재 = ①resolve 연결 카탈로그 id ②카탈로그 code=material_code ③동명(소문자·공백 제거) 매칭. 매칭 자재는 **수량을 물량 합산값으로 프리필**(중복 코드 합산, 소수 2자리), 배너로 "공종 n개 · 자재 m건 자동 선택" 안내 + 항목에 📐 뱃지.
- **물량 불러오기 공종 폴백**(addTakeoffRows): 행 trade 비면 `resolve.trade` 로 자동 선택 (v27 단가 주입에 공종 보강).

## 검증 (2026-07-19, Playwright 관통 · 격리팀 시드)
- 시드: 격리팀 + 카탈로그 7종(3분할 단가, 1종 code=WD-0001) + 스케치업 물량 3건(타일 24.5·목공 12·**공종빈값+WD-0001** 18.2).
- 자동 선택: 배너 "공종 3개 · 자재 2건" — 타일(동명 매칭·qty 24.5)·바닥(code 매칭·qty 18.2)·목공(공종만) 자동 체크, 가설/도장 비체크. 합계 3,081,400원 정확.
- 수동 추가 체크 2건 → 요약 4건 → 완료 → 공종 그룹별 행 추가(빈 초기행 대체) → [견적 생성] 저장 → API 검증: 4항목 trade/qty(24.5·18.2)/3분할/행합계(1,862,000·1,219,400)/catalog_id 전부 일치.
- 물량 불러오기 폴백: 공종 빈 WD-0001 행 → **바닥공사 그룹**으로 추가(프리픽스 규칙 시드 경유).
- 정리: 테스트 팀·유저·현장·카탈로그 잔존 0 (첫 실행 고아 포함).

---

# v30 — 사진 라이트박스·수정·알림 딥링크 · 매뉴얼 인앱 본문(노션 임포트)+탭 CRUD · 일정 공정→업체 필터 · 출력인원 자동 연동 (2026-07-19)

## 30-1 서버
- `PATCH /api/photos/:id` `{photo_date?, processes?(배열/문자열), memo?}` — 부분수정(COALESCE), 잘못된 날짜 400. 소유검증은 기존 requireChildOwned 마운트.
- `GET /api/sites/:id/labor?date=` 의 `schedules[]` 에 **`vendor`**(일정 협력업체명) 추가 → 출력인원 시공팀 프리필용.

## 30-2 UI — 현장사진
- **PhotoLightbox**: 썸네일 클릭 → 전체화면(1/N 카운터), ◀▶ 버튼+키보드(←/→/Esc), URL 캐시+이웃 프리로드, 상단 [↗원본][✏️수정][🗑삭제], 하단 날짜·공정칩·업로더·특이사항.
- **PhotoEditModal**: 촬영일/공정(다중 칩, 📅 일정 연동 칩 포함)/특이사항 수정 → PATCH. 라이트박스·카드 양쪽 진입.
- **카드 액션 재설계**(designsystems 가독성): hover 상단 그라데이션 + **백색 원형(ring) 아이콘 버튼** ✏️/🗑, 모바일 상시 노출(sm: hover 시).
- **알림 보내기**: 기존 문구 + `📱 앱에서 바로 보기: {origin}#/go/photos/{siteId}` **딥링크** 추가(+클라이언트 링크는 발급 시). `#/go/<tab>/<siteId>` 는 App 부팅 시 1회 파싱 → 로그인 후 해당 현장·탭 자동 이동 + 해시 정리(parseGoRoute/buildGoUrl).

## 30-3 UI — 매뉴얼
- **노션 원문 42건 임포트 완료**(Notion MCP → enhanced-markdown 변환: columns 평탄화·콜아웃→`> 💡` 인용·색상주석 제거·이미지→`🖼 (사진은 노션 원문에서 확인)` 플레이스홀더 79개/30페이지·PDF 첨부 표기) → interior_manuals.content 채움(기존 본문 있는 행 보호). **43/43 본문 보유.**
- **MdView**(경량 마크다운 렌더러): 제목/인용(앰버 블록)/리스트/번호/체크박스/굵게/링크/이미지(https만)/구분선/파이프표(모노). ManualViewModal 이 pre-wrap 대신 이걸로 렌더.
- **매뉴얼 탭 관리자 CRUD**: [+ 새 매뉴얼](저장 시 이 현장에 자동 선택), 카드 hover ✏️수정/🗑삭제(백색 원형 버튼), 삭제 확인 모달. 서버는 기존 requireAdmin CRUD 그대로. ManualEditModal 은 저장 행을 onSaved(saved) 로 반환, 본문 textarea 마크다운 안내.
- 열람은 항상 `GET /api/manuals/:id` 로 전체 본문 fetch.

## 30-4 UI — 일정·출력인원 연동
- **ScheduleForm 공정→협력업체 필터**: 공정명·업체 trade/kind 를 `/·,+공백` 토큰화(+'공사' 접미 제거) 양방향 부분일치 → 연관 업체만 datalist 노출 + "🔍 연관 업체 N개만 표시 중 · [전체 업체 보기]" 토글(연관 0개면 전체). vendors 전체 객체를 App→ScheduleTab→ScheduleForm 으로 전달.
- **출력인원 자동 연동**: 당일 일정 행 생성 시 일정의 vendor 명 → vendors 매칭 → 시공팀 select 자동 선택 + 기공/조공 일당 프리셋 자동 입력 + "📅 일정 업체 자동" 배지(저장된 기록 있으면 기록 우선). load deps 에 vendors.
...
## v31.1 — 사용자 피드백 반영 (2026-07-24)
- **가입 비밀번호 확인(교차검증)**: AuthScreen 회원가입에 `비밀번호 확인` 필드 추가 — 실시간 일치 표시(✓초록/✗빨강), 불일치 시 제출 버튼 비활성 + 서버 제출 차단.
- **상세 프로필 필드**(그룹웨어형): interior_users +4 `department`(부서)·`position`(직급)·`specialty`(담당 분야/전문)·`emergency_contact`(비상 연락처). 전부 선택 입력, `PATCH /api/me/profile` 확장. 나의 정보 모달에 입력란 추가. (staff 마스터 동기화 대상 아님 — 프로필 전용)
- **출력인원 `지원` 제외(되돌림)**: v31에서 `kind IN ('공사','지원')` 로 넓혔던 것을 사용자 확인에 따라 **`kind='공사'` 로 환원**('지원'은 인력 입력 대상 아님). "N일차/총M일" 배지는 공사 일정에 그대로 유지. 지원 배지 UI 제거.
- **비밀번호 변경(자발적)**: 나의 정보 › 🔒 비밀번호 [변경하기](v31에 이미 포함, 배포로 노출).
- interior_users 총 컬럼: v28(9)+v31(비번/약관 8)+v31.1(4) = 프로필·보안 확장.

---

# v32 — 일정 폼 개편 · 협력업체 시공/자재 분리 · 자재 발주 자동생성 · 사진 줌 (2026-07-25)

## 32-0 DB (ADD COLUMN IF NOT EXISTS — v1~v31 데이터 보존)
- `interior_schedule` +`vendor_material TEXT`(협력업체 자재). 기존 `vendor`=협력업체(시공).

## 32-1 일정 폼(ScheduleForm) 개편
- **공정 ↔ 작업명 순서 교체**: 공정을 먼저 선택 → 작업명 자동 채움. 작업명이 비어있거나 직전 공정값과 같을 때만 덮어씀 → 사용자가 뒤에 덧붙인 값 보존(예: 공정 '목공' → 작업명 '목공' → '목공-가벽시공'으로 수정 후 공정 바꿔도 유지).
- **상태 필드 제거 → 날짜 기준 자동**: 오늘<시작=예정, 시작≤오늘≤종료=진행, 오늘>종료=완료. 서버 `deriveScheduleStatus()`가 `rowToSchedule` 에서 매 조회마다 산출(저장값 무시) → 목록·캘린더·요약(doneCount) 항상 최신. 폼엔 자동 상태 배지만 표시.
- **계획비용 필드 제거**(컬럼은 유지 — 기존 데이터·거래처 상세 '계획 합' 보존).
- **협력업체 시공/자재 분리**: `협력업체(시공)`=vendor(kind에 '시공' 포함 업체만), `협력업체(자재)`=vendor_material(kind에 '자재' 포함 업체만). 각 공정 연관 필터 + [전체 보기] 토글. kind 데이터 없으면 전체 목록 폴백. (노션 import vendor.kind = '자재'/'시공'/'자재,시공')

## 32-2 자재 발주 자동생성 (스케치업 물량 × 자재상 단가)
- `POST /api/sites/:id/orders/auto-generate-material?replace=1` — takeoff(스케치업 CSV 물량) 각 품목을 팀 단가 카탈로그(판매처 vendor·자재단가 material_price 보유 행)와 **정규화 이름 완전일치→부분일치, 같은 공종 우선→최저단가**로 매칭 → 자재상(vendor)별 Σ round(qty×price) → 발주 1건씩 초안(memo 에 품목 내역, need_date=공정 min(start)−3). 반환 `{generated, matched, skipped, unmatched[], orders}`. 물량 없으면 404, 매칭 0이면 안내.
- **replace=1** 은 `auto_generated·대기·vendor≠''`(자재 초안)만 삭제 → 견적기반 초안(vendor='')·수동 발주 보존.
- UI: 발주 탭 툴바 **[📐 물량 → 자재 발주]**(항상 replace=true 재생성). 토스트에 생성/매칭/미매칭 수.

## 32-3 사진 줌 (ZoomableImage 공용 컴포넌트)
- PhotoLightbox(앱)·SimpleLightbox(클라이언트 공유) 모두 적용. **휠/더블클릭/핀치 확대(1~5배), 드래그 팬, 배율 1일 때 좌우 스와이프=사진 넘기기**. 우하단 −/배율%/+/[1:1] 컨트롤. 사진 바뀌면 줌 리셋. 좌우 ‹› 버튼은 z-10 로 위.
...
# v34 — 현장사진 일정별 표 · 내 업무 통합 뷰 · 자료 인라인 미리보기 + AI 문서 읽기 (2026-07-25)

## 34-1 현장사진 일정별(날짜별) 표
- PhotosTab 갤러리 카드에 **[갤러리]/[📅 일정별]** 토글. 일정별 표: 행 = {일정(공사/지원) 진행일 ∪ 사진 촬영일} 내림차순. 각 행 = **날짜(요일) + 그날 진행 공정(일정 겹침, procHex 색배지) + 사진 N장**. 오늘 배지. 행 클릭 → 그 날짜로 필터 + 사진 있으면 라이트박스(index 0), 없으면 필터만. schedTasks·autoProcsFor(기존) 재사용, 태스크당 400일 가드.

## 34-2 내 업무 통합(전체) 뷰
- MyTab 서브탭에 **[전체]** 추가(기본 선택). view='team'·status='진행' 로드 → **🙋 나에게 해당**(assignee_user_id=본인) / **👥 팀 (다른 담당)** 두 섹션 분리 렌더(각 건수·빈 안내). 헤더 '남은 할일'=내 배정 건수.

## 34-3 자료 인라인 미리보기 + AI 문서 읽기
- **DocPreviewModal**(size xl): 자료명 클릭/👁 → 모달. 이미지=`<img>`, PDF=`<iframe>`(브라우저 내장 뷰어, 다운로드 아님) 인라인 렌더. [↗ 새 탭] 병행. 미지원 형식은 안내+다운로드.
- **AI 읽기**: `POST /api/documents/:id/ai {question?}` — **PDF 는 원본 base64 를 gpt-4o file 입력으로 네이티브 전달**(pdf-parse 등 의존성 0), 이미지는 Vision(image_url=서명URL). 요약 프롬프트(종류/대상/항목·금액·수량·일정/특이사항) 또는 사용자 질문. 25MB 초과 413, PDF·이미지 외 415. 소유검증=requireChildOwned. 모달에 결과 + 후속 질문 입력.
- 검증: 실 PDF(9.2MB 제안서)로 gpt-4o 가 페이지별 실내용(라임스톤 데스크·대기실 65인치 TV·복도 트래버틴 등) 정확 요약. Playwright: 일정별 표(7/19 4장·7/25 오늘 2장, 공정 배지)→행 클릭 라이트박스 / 내업무 전체(나에게 1·팀 1) / 자료 PDF 인라인 렌더 22p + AI 읽기 요약. 콘솔 에러 0.

---

# v35 — 현장사진 표(사진 있는 날만) · 클라이언트 페이지 캘린더/사진 일정별/자료 인라인 (2026-07-25)

## 35-1 현장사진 일정별 표 = 사진 있는 날만
- PhotosTab dateRows: 일정 진행일 나열 제거 → **사진이 있는 날짜만**(photoCountByDate keys). 각 행 = 날짜(요일) + 그날 진행 공정(일정 겹침 배지) + 사진 장수, 클릭→라이트박스.

## 35-2 클라이언트 공유 페이지 (ClientView) 3종
- **ReadOnlyCalendar**(신규 공용 컴포넌트): 읽기전용 월 캘린더(드래그/수정 없음), 날짜 위 공정 막대(schedBarColor/kindPrefix, 첫 태스크 월로 초기 커서). 클라이언트 일정 탭에 **[📅 달력]/[목록] 토글**(달력 기본) → 공정 캘린더 공유.
- **클라이언트 사진 일정별 표**: [갤러리]/[📅 일정별] 토글. 사진 있는 날짜만, 날짜(요일)+진행 공정(공유 schedule 겹침)+장수, 행 클릭→SimpleLightbox.
- **클라이언트 자료 인라인 미리보기**: 자료명/[보기] → 모달(이미지 img·PDF iframe, 공유 payload 의 서명 url 직접 사용). [↗ 새 탭] 병행. **AI 읽기는 없음**(공개·무인증 페이지). 미리보기 불가 형식은 새 탭.
